...
int packet_queue_put(PacketQueue *q, AVPacket *pkt ,int handle) {
 ...
 //(media_handle_union_t *)handle
	...
	if (av_dup_packet(pkt) < 0){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"av_dup_packet failed");
		return -1;}

	pkt1 = av_malloc(sizeof(AVPacketList));
	if (!pkt1){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"av_malloc failed");
		return -1;
	}
	pkt1->pkt = *pkt;
	pkt1->next = NULL;
	...
	if(q->nb_packets == q->packet_max_num){
		...
		if(((media_handle_union_t *)handle)->stop_mark == 1){
			log_chris(ANDROID_LOG_ERROR ,TAG ,"==stop_mark ==1 ,and exit packet_queue_put");
			return -1;
		}
...
int packet_queue_get(PacketQueue *q, AVPacket *pkt ,int block ,int handle) {
	...
	for(;; ){
		...
		if(pkt1 == NULL){
			log_chris(ANDROID_LOG_ERROR ,TAG ," pkt1 == NULL");
		}
		if (pkt1) {
			...
			*pkt = pkt1->pkt;
			...
			//empty queue
			log_chris(ANDROID_LOG_ERROR ,TAG ,"empty queue ,and wait ,pkt1 == NULL");
			//display buffering
			call_java_onerror(g_obj ,100);

			pthread_cond_wait(&q->cond, &q->mutex);
			if(((media_handle_union_t *)handle)->stop_mark == 1){
				log_chris(ANDROID_LOG_ERROR ,TAG ,"==stop_mark ==1 ,and exit packet_queue_get");
				return -1;
			}
		}
	}
