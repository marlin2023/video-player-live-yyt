...
decode_audio_u * init_decode_audio(AVFormatContext *fmt_ctx ,decode_audio_u * decode_audio_var) {
	...
	if (decode_audio_var->audio_stream < 0) {
		log_chris(ANDROID_LOG_ERROR ,TAG ,"can not find audio stream ");
		return NULL;
	}
	...
	if ((decode_audio_var->ptr_audio_codec = avcodec_find_decoder(
		...
		log_chris(ANDROID_LOG_ERROR ,TAG ,"find audio decoder failed");
		return NULL;
	}

	// open audio decoder
	if (avcodec_open2(decode_audio_var->ptr_audio_codec_ctx, decode_audio_var->ptr_audio_codec ,NULL) < 0) {
		log_chris(ANDROID_LOG_ERROR ,TAG ,"open audio decoder failed");
		return NULL;
	}
	...
	//audio frame
	decode_audio_var->audio_decoded_frame = avcodec_alloc_frame();
	if(decode_audio_var->audio_decoded_frame == NULL){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"audio_decoded_frame alloc_frame failed");
		return NULL;
	}
	...
	log_chris(ANDROID_LOG_ERROR ,TAG ,"av_get_bytes_per_sample = %d" ,av_get_bytes_per_sample(decode_audio_var->ptr_audio_codec_ctx->sample_fmt) );
	...
	for(;;) {
		while(decode_audio_var->audio_pkt_size > 0) {
			...
			if( len < 0 ) { //decode failed ,skip frame
					log_chris(ANDROID_LOG_INFO ,TAG ,"Error while decoding audio frame");
			...
			if( got_frame ) {
					...
					if(decode_audio_var->ptr_audio_codec_ctx->sample_fmt == AV_SAMPLE_FMT_FLTP){
						...
						if(NULL == swr_ctx){ //init swrContext ,only one time
							...
							{
								log_chris(ANDROID_LOG_ERROR ,TAG ,"swr_init() for AV_SAMPLE_FMT_FLTP fail");
								return -1;
							}
						}//end init
					...
					return data_size;
					 ...
					 log_chris(ANDROID_LOG_INFO ,TAG ,"no data\n");
					 decode_audio_var->audio_pkt_size = 0;
					 if( pkt.data)
						av_free_packet(&pkt);
					 continue;
		...
		if(packet_queue_get(&decode_audio_var->audio_queue, &pkt ,decode_audio_var->finish_mark ,decode_audio_var->handle) < 0) {
			__android_log_print(ANDROID_LOG_ERROR, TAG, ".....queue get  audio failed.,decode_audio_var->file_over =%d...",decode_audio_var->finish_mark);
