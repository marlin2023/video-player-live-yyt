...
static int interrupt_cb(void *ctx)
{
	...
	log_chris(ANDROID_LOG_INFO ,TAG ,"in interrupt_cb function ,ret = %d ,stop_mark = %d" ,ret ,media_handle->stop_mark);
...
void *play_audio_thread(void *opaque){
	...
	log_chris(ANDROID_LOG_ERROR ,TAG ,"in xplayer_play ,before opensl  ,handle = %d" ,media_handle);
	//in this function ,new a thread to play audio
	opensl_play_audio(media_handle->decode_audio_var ,media_handle->ptr_format_ctx);
	log_chris(ANDROID_LOG_ERROR ,TAG ,"==play audio thread over ");
...
PLAYER_HANDLE init_media(char *media_src ) {

	media_handle_union_t * media_handle;
	if ((media_handle = malloc(sizeof(media_handle_union_t))) == NULL) {
		log_chris(ANDROID_LOG_INFO ,TAG ,"memory allocate failed");
...
//        return call_java_onerror(g_obj ,MALLOC_MEMORY_FAIL);
//    }
	log_chris(ANDROID_LOG_ERROR ,TAG ,"before avformat_open_input ");
...
//	media_handle->ptr_format_ctx->interrupt_callback.opaque = (void *)media_handle;
	...
	if( ret < 0){
		log_chris(ANDROID_LOG_INFO ,TAG ,"avformat_open_input failed ,ret = %d" ,ret);
	...
	media_handle->ptr_format_ctx->max_analyze_duration  = 6000000 ;//5000000 ->5M ;
	log_chris(ANDROID_LOG_ERROR ,TAG ,"before avformat_find_stream_info ");
	if (avformat_find_stream_info(media_handle->ptr_format_ctx ,NULL) < 0) {
		log_chris(ANDROID_LOG_INFO ,TAG ,"av_find_stream_info failed in init function ");
		return call_java_onerror(g_obj ,AVFORMAT_FIND_STREAM_INFO);
	}
	log_chris(ANDROID_LOG_ERROR ,TAG ,"before av_dump_format ");
	...
	if(media_handle->decode_audio_var == NULL){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"decode_audio_var malloc failed ");
		return call_java_onerror(g_obj ,MALLOC_MEMORY_FAIL);
	}
	if(init_decode_audio(media_handle->ptr_format_ctx ,media_handle->decode_audio_var) == NULL){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"init_decode_audio init failed ");
		return call_java_onerror(g_obj ,INIT_DECODE_AUDIO_FAIL);
	}

	log_chris(ANDROID_LOG_ERROR ,TAG ,"after init_decode_audio ...");
	/*	init video stream 	*/
	media_handle->decode_video_var = (decode_video_u *)malloc(sizeof(decode_video_u));
	if(media_handle->decode_video_var == NULL){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"decode_video_var malloc failed ");
		return call_java_onerror(g_obj ,MALLOC_MEMORY_FAIL);
	}
	log_chris(ANDROID_LOG_ERROR ,TAG ,"before init_decode_video...");
	if (init_decode_video(media_handle->ptr_format_ctx ,media_handle->decode_video_var) == NULL){	//init video decode
		log_chris(ANDROID_LOG_ERROR ,TAG ,"init_decode_video failed...");
		return call_java_onerror(g_obj ,INIT_DECODE_VIDEO_FAIL);
	}
	log_chris(ANDROID_LOG_ERROR ,TAG ,"after init_decode_video...");

	/*	init opengl es varibale */
	if( init_opengl_variable((int)media_handle) < 0){
		log_chris(ANDROID_LOG_ERROR ,TAG ,"init_opengl_variable failed...");
	...
	__android_log_print(ANDROID_LOG_INFO,"chris_magic", "in init ,media_handle->seek_mark =%d.,media_handle = %d" ,media_handle->seek_mark ,media_handle);
...
void xplayer_play(PLAYER_HANDLE handle ){
	...
	log_chris(ANDROID_LOG_INFO ,TAG ," in xplayer_play");
	media_handle_union_t * media_handle = (media_handle_union_t *) handle;
	if (media_handle == NULL) {
		log_chris(ANDROID_LOG_ERROR ,TAG ,"media handle is null");
		return;   //ok ????
