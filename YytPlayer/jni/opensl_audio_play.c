...
void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context){
	...
	if( ( (media_handle_union_t *)g_decode_audio->handle )->video_prepared != 1){
		...
		while(1){
			usleep(500);
			log_chris(ANDROID_LOG_ERROR ,TAG ,"===video prepared ,prepared = %d"  ,( (media_handle_union_t *)g_decode_audio->handle )->video_prepared);
			if(( (media_handle_union_t *)g_decode_audio->handle )->video_prepared == 1){
				log_chris(ANDROID_LOG_ERROR ,TAG ,"===video prepared  ....ok!");
...
void opensl_play_audio(decode_audio_u *decode_audio_var ,AVFormatContext *fmt_ctx){
	...
	log_chris(ANDROID_LOG_ERROR ,TAG ,"in opensl_play_audio ,before createEngine");
	if(create_engine_mark == 0){
		//init opensl
		createEngine();
		log_chris(ANDROID_LOG_ERROR ,TAG ,"in opensl_play_audio ,after createEngine");
	}
	createBufferQueueAudioPlayer();
	log_chris(ANDROID_LOG_ERROR ,TAG ,"in opensl_play_audio ,before bqPlayerCallback()");
	...
	for(;;){
		...
		//getchar();
		if(loop_mark == 1){
			log_chris(ANDROID_LOG_ERROR ,TAG ,"loop_mark = 1,exit opensl_play_audio");
			loop_mark = 0;
			break;
		}
		usleep(200);
